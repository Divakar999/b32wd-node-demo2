...
module.exports.updateEmployee=async (req,res,next)=>{
  try{
    var data=await mongo.selectedDB.collection("employee").findOneAndUpdate({_id :ObjectId(req.params.id)}, {$set:{...req.body}},{retuenNewDocument:true})
     res.send(data);
  }
  catch(err){
    //  console.log(err);
...
module.exports.updateEmployeeRole=async (req,res,next)=>{
  try{
    var data=await mongo.selectedDB.collection("employee").updateOne({_id :ObjectId(req.params.id)}, {$set:{role:req.body.role,salary:req.body.salary}})
     res.send(data);
  }
  catch(err){
    //  console.log(err);
